...
'''
#SCRIPT DE CLASSIFICACAO POR BACIA — versão 3 (VC3_bal)
#Produzido por Geodatin - Dados e Geoinformacao
#DISTRIBUIDO COM GPLv2
#
# Diferença em relação a VC2:
#   - down_samples_ROIs usa limites por bacia lidos de
#     dados/reprocess_v2/dict_qt_limit_v2_por_bacia.json
#     (gerado por src/validations/acccuracy/gerar_dict_qt_limit_v2.py)
#   - version = 2  → assets nomeados fm-v_2
#   - lista de bacias restrita às 36 bacias com regressão de acurácia
'''
...
class ClassMosaic_indexs_Spectral(object):
    ...
    def down_samples_ROIs(self, rois_train, nbacia):
        # Limites v1 como fallback para bacias não presentes no JSON
        # dictQtLimit_v1 = {
        #     '3': 520, '4': 1200, '12': 300, '15': 970,
        #     '19': 180, '21': 200, '22': 400, '29': 200,
        #     '33': 100, '36': 260
        # }

        dictQtLimit_v1 = {
            '3': 250, '4': 2500, '12': 50, '15': 1700,
            '19': 100, '21': 60, '22': 150, '29': 50,
            '33': 50, '36': 100
        }
        limits_bacia = self.dictQtLimitV2.get(str(nbacia), {})
        # dictQtLimit = {k: limits_bacia.get(k, v) for k, v in dictQtLimit_v1.items()}
        dictQtLimit = dictQtLimit_v1
    ...
    def iterate_bacias(self, _nbacia, myModel, makeProb, process_mosaic_EE, lista_years):        
        ...
        for nyear in self.lst_year[:]:  # 
            ...
            if nyear not in lista_years:                
                ...
                # Override mínimo para bacia/ano com histórico de timeout
                dictAnosSimples = {
                    '7564': [1993, 2010]
                }
                if _nbacia in dictAnosSimples and nyear in dictAnosSimples[_nbacia]:
                    pmtroClass['numberOfTrees'] = 6
                    pmtroClass['shrinkage']     = 0.05
                    bandas_imports = self.lst_feat_select[:15]
                    print(f"  [SIMPLES] bacia={_nbacia} ano={nyear} → {len(bandas_imports)} feats, {pmtroClass['numberOfTrees']} árvores")
...
registros_proc = "registros/lsBaciasClassifyfeitasv_2.txt"
...
# 36 bacias com regressão de acurácia — reprocessamento v2 (balanceamento por bacia)
nameBacias = [
