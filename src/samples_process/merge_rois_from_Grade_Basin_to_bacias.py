...
param = {
    'asset_rois_grid1': {'id' : 'projects/mapbiomas-workspace/AMOSTRAS/col11/CAATINGA/ROIs/ROIs_byGradesInd'},
    # 'asset_rois_grid2': {'id' : 'projects/nexgenmap/SAMPLES/Caatinga/ROIs'},
    'asset_bacias_buffer' : 'projects/mapbiomas-workspace/AMOSTRAS/col9/CAATINGA/bacias_hidrografica_caatinga_49_regions',
    'asset_output': 'projects/mapbiomas-workspace/AMOSTRAS/col11/CAATINGA/ROIs/ROIs_byBasinInd',
...
def ask_ifGrid_ROIS_saved( dict_asset, printName, listtoLoad): 
    ...
    getlstFeat2 = ee.data.listAssets(dict_asset['id'])    
    print(f" we have {len(getlstFeat2)} featshp of ROIs")
    assets_lista = getlstFeat2['assets']
    print(" assets_lista    ", assets_lista[:2])
    # sys.exit()
    sizeFeat = 0
    featAllRois = ee.FeatureCollection([])
    cc = 0
    lstPresent =[]
    for idAsset in tqdm(assets_lista):  
        print(idAsset)     
        path_ = idAsset['id']
